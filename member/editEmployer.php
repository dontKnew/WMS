<?php require('header.php');
    if(isset($_GET['id'])){
        $sql = "SELECT * FROM employers_wms WHERE employer_id = '". $_GET['id']."'";
        ...
        if($result==true){
            ...
            if(isset($data)){
                ...
                <form id="form_id" class="w-full"  action="employers_info.php?id='.$data['employer_id'].'" autocomplete="off" method="POST">
                <span class="errorAddemployer"></span>
                    ...
                    <div class="mt-5 text-center">
                        <input type="submit" name="updateEmployer" value="UPDATE" class="cursor-pointer mx-2 bg-green-500 hover:bg-green-800 text-white font-bold py-2 px-4 rounded" />
                        <a href="employers_info.php" class="close-modal cursor-pointer bg-red-200 hover:bg-red-500 text-red-900 font-bold py-2 px-4 rounded">
                            CANCEL
                        </a>
                    </div>
                </form>
                ...
                // echo "<script> location.href='employers_info.php'; </script>";
                echo $conn->error;
            }
        }else{
            echo $conn->error;
